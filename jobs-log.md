# Job Scout — Welcome Back Report

Running master file. The weekly scan appends here; existing entries are never rewritten. Read top-to-bottom on return.

> [!note] Entries up to 2026-09-02 were reformatted into the bulleted layout below. No content was changed — only the structure.

---

## Run history
<!-- Newest first. Format:
### YYYY-MM-DD · run N
- 📥 Alerts read: N
- ✅ New: 🔥 N · ⏳ N
- **Added** / **Hard-filtered** / **Discarded** — one role per bullet, one line each
- ⚠️ Issues: one per bullet
Omit empty sub-sections. -->

### 2026-09-02 · run 5
- 📥 Alerts read: 0 _(Gmail IMAP error — web sources only)_
- ✅ New: 🔥 1 · ⏳ 1
- **Added**
  - 🔥 **Blinq** — Staff Product Designer · 80/100 · Melbourne hybrid 3 days · perm · consumer networking startup
  - ⏳ **Blinq** — Senior Product Designer · 72/100 · Melbourne hybrid · perm · enrich next run
- **Hard-filtered**
  - **SafetyCulture** — Product Designer Design Systems · Sydney hybrid, no AU-remote confirmed
- **Discarded**
  - **Sapia.ai** — no product design roles found
  - **Harrison.ai** — no product design roles found (open positions are non-design)
- ⚠️ Issues
  - Gmail IMAP error (bad arguments) — web sources only
  - Lever and startup.jobs returned 403 — Blinq roles scored from search snippets only

### 2026-08-31 · run 4
- 📥 Alerts read: 0 _(Gmail not configured)_
- ✅ New: 🔥 0 · ⏳ 1
- **Added**
  - ⏳ **Bupa** — Design Lead, CRM Transformation · 55/100 · Melbourne contract to Jan 2027
- **Discarded**
  - **EatClub** — Senior PD, remote · ~42/100 · hospitality domain not preferred, perm, mild execution tilt
  - **Bupa** — Principal Design Researcher · wrong discipline (researcher, not product designer)
  - **Lorikeet** / **New Forests** — no product design roles found
- ⚠️ Issues
  - Fetch cap hit (6/6 — all 404 or empty)
  - Scored from search snippets only

### 2026-08-30 · run 3
- 📥 Alerts read: 0 _(Gmail not configured)_
- ✅ New: 🔥 0 · ⏳ 2
- **Added**
  - ⏳ **Atticus** — Senior PD · 60/100 · Melbourne/AU remote · $150–180k perm · legal doc-review scale-up
  - ⏳ **Nintex** — Senior PD · 65/100 · Melbourne remote · AI-assisted workflow automation, discovery-explicit
- **Hard-filtered**
  - **Eucalyptus** — Lead PD · Sydney, no remote confirmed
  - **Dovetail** — Senior PD · Sydney + 4 days onsite
  - **Education Perfect** — PD · below senior
  - **Deputy** — Senior PD · Sydney hybrid, no AU remote confirmed
- 🎯 Targets checked: Dovetail, Eucalyptus, Atticus, Deputy
- ⚠️ Issues
  - Fetch cap respected (4/6 used)

### 2026-06-21 · run 2
- 📥 Alerts read: 18
- ✅ New: 🔥 0 · ⏳ 3
- **Added**
  - ⏳ **Mable** — Senior PD · disability/aged care healthtech · perm · rate unknown
  - ⏳ **Atlassian** — Lead PD · enterprise B2B SaaS · full JD not retrieved, enrich next run
  - ⏳ **Heidi** — Head of PD · leadership, not IC (distinct from the logged 🔥 Senior PD)
- **Hard-filtered**
  - **Easygo** — gambling
  - **Pulse Identity** — rate $7–10k/month, below threshold
- **Discarded**
  - **Airwallex** — Product Design Manager · management, not IC
  - **AustralianSuper** — Senior Service Designer · wrong discipline
  - **Up Bank** — brand designer only, no PD role
- ⚠️ Issues
  - Most WebFetch calls 403/redirect — scored from search snippets

### 2026-06-17 · run 1
- 📥 Alerts read: 10
- ✅ New: 🔥 2 · ⏳ 3
- **Added**
  - 🔥 **Amber** — confirmed open (calibration match)
  - 🔥 **Cadmus** — confirmed open (calibration match)
  - ⏳ **Canva** — Lead PD, Design Systems
  - ⏳ **UniSuper** — 12-month contract
  - ⏳ **Airwallex** — Staff PD · enrich next run, full JD needed
- **Skipped**
  - **Heidi Health** — rechecked as target company, already logged
- **Hard-filtered**
  - **Easygo / KICK** — gambling
  - **Aristocrat** — gambling + Adelaide
  - **Sitemate** — $90–120k salary
  - **Mathspace** — level unclear/entry
- ⚠️ Issues
  - AGL Digital PD appeared in SEEK alerts but insufficient JD detail to score — monitor next run

### 2026-06-15 · setup validation
- 🧪 Manual validation run, not a scheduled run
- 🎯 Target-company pages tested: Heidi, Dovetail
- ✅ New: 🔥 1 · ⏳ 0
- **Notes**
  - Pipeline verified end-to-end
  - Heidi careers page parsed cleanly; strong-fit role found
  - Dovetail had no current product-design role (correctly logged nothing)
  - Counter left at 0 — the first real run starts the count

Total runs: 0
Last digest sent after run: —

---

## 🔥 Live & strong fit (apply now)
<!-- ### 🔥 [Role] — [Company]
**NN/100** · Contract/Perm · [Location] · Posted YYYY-MM-DD
**Why it fits** — one reason per bullet
**Watch-outs** — one reservation per bullet (omit if none)
**Practicals** — 💰 rate · 📅 start · 👤 contacts · 🎯 angle · 🔗 link -->

### 🔥 Staff Product Designer — Blinq
**80/100** _(perm −10 applied; scored from snippets only — enrich next run)_ · Perm · Melbourne hybrid (3 days/wk) · Found 2026-09-02

**Why it fits**
- Staff-level scope: leads design across Blinq's most important product surfaces.
- End-to-end ownership — "from insight to execution" across iOS, Android and Web.
- Melbourne HQ startup, raised $45M+, doubling ARR — high visibility, clear impact.
- Small design team signals broad senior-IC remit.
- Growth-stage scale-up matches your Culture Amp and Informed Decisions phase experience.

**Watch-outs**
- Perm only — handicap applied.
- Consumer networking domain (digital business cards) — not a public-good or health area.
- No AI-native or 0→1 language found in snippets.
- 3 days/wk in office — at the maximum hybrid threshold.
- Full JD not retrieved (Lever 403) — score is provisional.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown
- 📅 Start: not stated — verify; flag if before 9 Sep 2026.
- 👤 Contacts: no named design leadership confirmed.
- 🎯 Angle: ex-Culture Amp AI designer with end-to-end IC ownership and design system track record; Blinq's staff-level scope maps to your cross-functional seniority at Informed Decisions.
- 🔗 Link: https://jobs.lever.co/blinq/8698989f-1f22-4b79-907d-3cc15eab783d | https://blinq.me/careers

### 🔥 Senior Product Designer — Heidi Health
**92/100** _(perm −10 applied)_ · Perm · Melbourne/Sydney · Posted 2026-02-17 _(found in setup validation run)_

**Why it fits**
- Bullseye against your calibration set — this *is* your "would apply" Heidi example.
- AI-native clinical health product — maps directly to your Babylon/Unmind and Culture Amp AI background.
- End-to-end ownership: design, prototype and ship.
- Explicitly explores "what AI clinical software should look and feel like".
- Obsessive craft bar; design system framed as "a floor not a ceiling".
- Melbourne/Sydney based.
- Honest-ad bonus: has the "if you answer no, this isn't for you" section.
- Target-company match.

**Watch-outs**
- Research is deliberately de-emphasised — not penalised, per your calibration.

**Practicals**
- 💰 Rate/salary: not stated; benefits + "serious equity" listed. ⚠️ rate unknown
- 📅 Start: open now. ⚠️ starts before ~9 Sep availability — worth an early conversation regardless
- 👤 Contacts: no named contact confirmed.
  - A **Head of Product Design (Melbourne)** role is open concurrently — design leadership is being built, so the eventual HoD is your future manager.
  - Find current Heidi designers via LinkedIn before applying.
- 🎯 Angle: ex-Culture Amp/Babylon/Unmind designer who's shipped AI-native and health products; reference your AI Coach work.
- 🔗 Link: https://www.heidihealth.com/careers/d4fca974-4af4-4dba-8957-1296ab295613

### 🔥 Staff/Principal Product Designer — Amber Electric
**75/100** _(perm −10 applied)_ · Perm · Melbourne · Posted ~Jun 2026

**Why it fits**
- Your exact "Textbook 🔥" calibration example.
- AU energy scale-up (~80k customers) — real-time wholesale electricity plus home battery, solar and EV automation.
- Hiring the most senior IC designer on the team.
- Brief is strategic ambiguity: "deployed on Amber's most strategically important problems — the ones that are too big, too cross-functional, or too consequential to be absorbed into a single stream."
- End-to-end ownership in a climate/energy public-good domain.
- Your Culture Amp AI portfolio and Informed Decisions design system work are direct proof points.

**Practicals**
- 💰 Rate/salary: not shown on the current posting. Calibration reference cites $180–240k + options — confirm on application. ⚠️ rate unconfirmed on current listing
- 📅 Start: role open now. ⚠️ starts before ~9 Sep availability — worth an early conversation
- 👤 Contacts: no named design leadership confirmed.
  - Vida Fox — appears in LinkedIn search for Amber design roles; verify current title before reaching out. https://www.linkedin.com/in/vida-fox/
  - Dan Adams — Co-founder & Co-CEO. https://au.linkedin.com/in/dan-adams-615b72a4
- 🎯 Angle: "Ex-Culture Amp AI designer + design system builder at Informed Decisions — both map directly to the strategic, cross-functional problems the Staff/Principal PD is being hired to solve."
- 🔗 Link: https://jobs.lever.co/amberelectric/231ccb7e-b488-45a3-b4f7-7640fe0a7378 | https://www.amber.com.au/amber-careers

### 🔥 Senior Product Designer — Cadmus
**75/100** _(perm −10 applied)_ · Perm · Melbourne hybrid · Posted 2026-05-18

**Why it fits**
- Your "Textbook 🔥" calibration example.
- Melbourne edtech scale-up (founded 2015); platform shapes assessment for millions of students and tens of thousands of academics.
- Owns core platform areas end-to-end, including design system evolution.
- Complex academic workflow design: grading, rubric creation, analytics.
- User research is part of the role.
- Explicit "what this role is NOT" section earns the honest-ad bonus.
- "Design is the mechanism of academic integrity" — same framing as your Cadmus calibration example.
- Your Informed Decisions institutional-workflow experience and Culture Amp B2B design system work are directly relevant.
- Melbourne hybrid fits.

**Practicals**
- 💰 Rate/salary: not disclosed. ⚠️ rate unknown
- 📅 Start: active. ⚠️ starts before ~9 Sep availability — worth an early conversation
- 👤 Contacts: no named design leadership confirmed publicly.
  - A Director of Product (APAC) role is also open — this person will likely sit above this designer. https://au.linkedin.com/jobs/view/director-of-product-apac-at-cadmus-3409137333
- 🎯 Angle: "Ex-designer on complex institutional B2B workflows (Culture Amp HR AI, Informed Decisions gov data) with design system ownership — both map to Cadmus's craft-led, workflow-transformation brief."
- 🔗 Link: https://apply.workable.com/cadmus-io/j/7537B56A80

---

## ⏳ Live & maybe
<!-- Same shape as 🔥 above, lower score (50–74). -->

### ⏳ Senior Product Designer — Blinq
**72/100** _(perm −10 applied; scored from snippet only — enrich next run; see also 🔥 Staff PD above)_ · Perm · Melbourne hybrid · Found 2026-09-02

**Why it fits**
- Same company as 🔥 Staff PD — Blinq Melbourne startup, raised $45M+.
- Senior level with broad product design remit assumed from company context.
- Growth-stage scale-up signals clear IC impact opportunity.

**Watch-outs**
- Full JD not retrieved — score is provisional.
- Perm only — handicap applied.
- Consumer networking domain — not preferred public-good area.
- Two Blinq roles open concurrently — verify scope before applying to both.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown
- 📅 Start: not stated — verify.
- 👤 Contacts: no named contact found.
- 🎯 Angle: same as 🔥 Staff PD above.
- 🔗 Link: https://jobs.blackbird.vc/companies/blinq/jobs/71592596-senior-product-designer | https://designproject.io/jobs/jobs/senior-product-designer-at-blinq-cagx4g

### ⏳ Lead Product Designer (Design Systems) — Canva
**60/100** _(perm −10 applied)_ · Perm · Posted ~Jun 2026

**Why it fits**
- Canva's Design Systems team is building a world-class system for consistent experiences at scale.
- Owns design system audit, component evolution, the 2026 roadmap, and advocacy.
- End-to-end ownership of a complex craft surface.
- Your design system work at Informed Decisions is a direct match.
- Prestigious AU tech company.

**Watch-outs**
- Large enterprise (3000+ employees, ~$40B valuation) — no startup bonus.
- No public-good, health or gov domain.
- Not discovery-led.
- Perm handicap applied.

**Practicals**
- 💰 Rate/salary: not listed. Glassdoor/Levels.fyi suggest $150–270k+ for design roles at Canva. ⚠️ rate unconfirmed on listing
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no specific hiring manager identified. Search Canva LinkedIn for Design Systems team leads before applying.
- 🔗 Link: https://www.lifeatcanva.com/en/jobs/967b381d-187e-43f2-8f7e-466e94f038d7/lead-product-designer-design-systems/

### ⏳ Senior Experience Designer — UniSuper
**60/100** · Contract 12m · Posted ~Jun 2026

**Why it fits**
- Superannuation fund for university employees (~500k members), Digital & CX team.
- Explicitly includes discovery — "lead discovery and design of exceptional digital experiences".
- Cross-functional collaboration and end-to-end delivery across web and mobile.
- Contract rather than perm — a positive, and no perm handicap.

**Watch-outs**
- Large institutional org — no startup bonus, some design-immaturity risk in large financials.
- 60% in office (≈3 days/wk) — at the hard limit, just passes.
- Financial services is not your preferred domain.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown — 12-month contract, daily rate likely negotiable
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no named hiring manager confirmed. Role sits in Marketing & Growth / Digital & CX — search UniSuper LinkedIn for design leadership.
- 🔗 Link: https://au.talent.com/view?id=609308522469400181 | https://au.indeed.com/viewjob?jk=4a2135bd6dc01a4c

### ⏳ Staff Product Designer — Airwallex
**52/100** _(perm −10 applied; enrich next run — full JD needed)_ · Perm · Melbourne hybrid · Posted Jun 2026

**Why it fits**
- Global fintech/payments platform, founded in Melbourne; 2000+ staff across 26 offices.
- Staff level signals strategic IC ownership and cross-functional scope.
- Melbourne hybrid.

**Watch-outs**
- Fintech/payments is not your preferred public-good domain.
- No evidence of discovery, 0→1 or AI-native language in snippets.
- Large enterprise — no startup bonus.
- Perm penalty applied.
- Multiple variants open (Staff PD, Staff PD UI/UX, Staff PD Design Systems) — unclear which matches the Indeed alert.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown — Airwallex offers RSUs + bonuses per careers page
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no named hiring manager identified. Check the Airwallex careers page for the specific role variant before applying.
- 🔗 Link: https://careers.airwallex.com/job/2e2b7832-9e48-45da-ab0e-030708f0a45b/staff-product-designer/ | https://careers.airwallex.com/

### ⏳ Senior Product Designer — Mable
**50/100** _(perm −10 applied; enrich next run for full honest-ad check)_ · Perm · Posted ~Jun 2026

**Why it fits**
- Australia's largest disability and aged care support marketplace (200+ staff, 3m+ support hours/year).
- Health/public-good domain with direct resonance to your Babylon and Unmind background.
- End-to-end design in cross-functional squads.
- Includes user research, usability testing, workshopping, concept development and validation.
- UX + UI in Figma; "lean and human-centred design approach" framing.
- Broad design team (UI, content, product, service, strategic designers) suggests design maturity.

**Watch-outs**
- Perm penalty applied.
- Salary not stated; Glassdoor median ~$119k is below the $140k threshold — keep and flag until confirmed or denied.
- Ad lacks an explicit "what this is NOT" section — no honest-ad bonus.
- 3–5 year minimum understates your seniority.
- "Decision-makers and action" framing is a mild speed-culture signal.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown — Glassdoor estimate ~$119k; if official salary is below $140k this becomes a hard-filter reject. Confirm on application.
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no named hiring manager confirmed. Check Mable LinkedIn for design leadership.
- 🔗 Link: https://work180.com/en-au/for-women/employer/mable/job/396907/senior-product-designer | https://mable.recruitee.com/

### ⏳ Head of Product Design — Heidi
**N/A** — Head-of / leadership role; logged per criteria, not scored as an IC fit · Perm (full-time) · Sydney-based, remote listed · Posted ~Jun 2026

**Why it fits**
- Clinical AI health product — a calibration-set perfect domain match.
- The Senior PD at Heidi is already logged as 🔥.
- This parallel opening signals Heidi is scaling its design leadership.
- The eventual Head is your future manager if you take the Senior PD role.
- If you're open to design leadership rather than IC, arguably the strongest management role on the radar: clinical AI, honest design culture, remote-friendly.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no named contact confirmed. Search Heidi LinkedIn for current design or product leadership before applying.
- 🔗 Link: https://www.heidihealth.com/careers _(specific posting URL not extracted — check directly)_

### ⏳ Senior Product Designer — Nintex
**65/100** _(perm −10 applied; enrich next run)_ · Perm · Melbourne + AU remote · Posted unknown _(found 2026-08-30)_

**Why it fits**
- Global process automation platform — low-code/no-code plus AI-assisted workflow capabilities.
- Role explicitly spans "discovery through delivery and iteration" — full end-to-end ownership.
- Designing for automation builders, logic flows, data mapping, integrations and AI-assisted capabilities.
- Staff and Principal designers already in place — mature design function.
- High autonomy with senior mentorship.
- B2B SaaS, product-craft focused.

**Watch-outs**
- Workflow automation is not your closest domain (vs health/AI/HR).
- No salary stated — no honest-ad bonus.
- Perm penalty applied.
- AI framed as "AI-assisted" rather than fully AI-native.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown
- 📅 Start: unknown — verify. Flag if before 9 Sep 2026.
- 👤 Contacts: no named hiring manager found.
- 🎯 Angle: ex-Culture Amp AI designer with end-to-end ownership and a design system track record; Nintex's AI-assisted workflow design maps directly to your Culture Amp AI Coach experience.
- 🔗 Link: https://www.nintex.com/careers/jobs/?gh_jid=7648659 | https://builtinmelbourne.com/job/senior-product-designer/8648914

### ⏳ Senior Product Designer — Atticus
**60/100** _(perm −10 applied; enrich next run)_ · Perm · Melbourne hybrid or AU remote · Posted unknown _(found 2026-08-30)_

**Why it fits**
- Melbourne scale-up (founded 2017); document fact-checking platform used by AU/UK law firms and listed companies to verify disclosures pre-publication.
- Full end-to-end ownership: "lead design projects from inception to execution, collaborate with users and engineers, create elegant solutions to complex problems."
- Small, high-performance team with multiple Senior Designers.
- Melbourne hybrid or AU remote.
- Salary range stated — honest-ad bonus applied.

**Watch-outs**
- Legal domain is not your strongest (vs health/AI/HR).
- No explicit AI-native or discovery language in snippets.
- Perm penalty applied.
- No formal Head of Design — reports to co-founders, so design maturity is unclear.

**Practicals**
- 💰 Rate/salary: $150k–180k AUD (perm, stated). Passes the $140k hard filter. ✓
- 📅 Start: unknown — verify. Flag if before 9 Sep 2026.
- 👤 Contacts: no named hiring manager confirmed.
  - Senior Product Designers Dominic Bonnice and Patrick Campbell appear in org listings — peers, not hiring contacts. Do not cold-contact.
- 🎯 Angle: ex-Onfido/Babylon/Culture Amp designer with scale-up pedigree and design system ownership (Informed Decisions) — both map to Atticus's craft-led, complex B2B product brief.
- 🔗 Link: https://apply.workable.com/atticustech/j/8299AD3AD8

### ⏳ Lead Product Designer — Atlassian
**~50/100** _(estimated from snippets; full JD not retrieved — enrich next run)_ · Perm · Melbourne hybrid or AU remote · Posted ~Jun 2026

**Why it fits**
- Global B2B SaaS enterprise, 10,000+ staff, Team Anywhere distributed culture.
- Lead level signals strategic IC scope and cross-functional design ownership.
- Team Anywhere means Melbourne hybrid or AU remote, which fits.
- B2B SaaS domain matches your Culture Amp and Informed Decisions work.

**Watch-outs**
- Large enterprise — no startup/scale-up bonus, enterprise penalty applied.
- Full JD not retrieved; careers page returned empty (JS-rendered or auth-gated).
- Product area unknown — without knowing which product (Jira/Confluence/Atlas), domain and discovery alignment can't be confirmed.
- Score is a rough estimate; could go higher or lower once the full ad is read.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown — Atlassian salaries typically strong; confirm on application
- 📅 Start: active. ⚠️ starts before ~9 Sep availability
- 👤 Contacts: no named hiring manager confirmed. Check Atlassian LinkedIn for the relevant design team.
- 🔗 Link: https://www.atlassian.com/company/careers/details/15945

### ⏳ Design Lead – CRM Transformation — Bupa
**55/100** · Contract (fixed-term to Jan 2027) · Melbourne · Found 2026-08-31

**Why it fits**
- AU/global health insurance, 6000+ AU employees.
- Design Lead on the Apollo CRM transformation program — a Microsoft Dynamics 365 rollout affecting customer and employee experiences.
- Explicitly discovery-led: "lead discovery, customer experience, and service design activities from discovery through to delivery."
- HCD/design thinking methodology stated.
- Cross-functional stakeholder remit: product, technology, business, risk, legal.
- Health domain maps to your Babylon and Unmind background.
- Fixed-term contract to Jan 2027 — no perm penalty.

**Watch-outs**
- Large enterprise — no startup bonus.
- CRM transformation context limits 0→1 or AI-native craft; more service/CX redesign than greenfield product work.
- No salary or rate stated.
- Design team structure and maturity unclear from snippets alone.
- Scored from search snippets only — direct page URLs all 404, so the role may be recently posted or gated.

**Practicals**
- 💰 Rate/salary: not stated. ⚠️ rate unknown
- 📅 Start: active. ⚠️ may need to start before or near 9 Sep availability — confirm with hiring team
- 👤 Contacts: no named design hiring manager confirmed publicly.
- 🎯 Angle: ex-Babylon/Unmind health designer with complex B2B end-to-end ownership (Culture Amp AI suite, Informed Decisions flagship redesign + design system) — maps directly to the CRM transformation brief's cross-functional, discovery-led scope.
- 🔗 Link: https://careers.bupa.com.au/job/melbourne/design-lead-crm-transformation/40796/42455981824 _(returned 404 on fetch — verify directly on careers.bupa.com.au)_

---

## 💀 Expired but relevant — outreach targets
<!-- ### 💀 [Role] — [Company] — closed YYYY-MM-DD
**Why it was a fit** — bullets
**Who to contact** — 👤 name/title/URL · 🎯 angle -->

_None yet._

---

## 🏢 Companies hiring repeatedly (warm signal even with no open role)

### Heidi
- Senior PD (🔥, logged Feb 2026) and Head of Product Design (⏳, logged Jun 2026) open concurrently.
- Design team scaling aggressively — two roles active simultaneously.
- The HoD hire means the Senior PD will have a named design manager soon.

### Airwallex
- 4+ design roles advertised concurrently across Jun 2026: Staff PD, Staff PD Design Systems, Staff PD UI/UX, Senior PD, Product Design Manager.
- Repeated Indeed alerts over multiple days.
- Major hiring push — the role may be hard to fill, or the team is expanding significantly.

---

## 📇 Master contact list

| Name | Title | Company | Source role | Angle |
|---|---|---|---|---|
| Vida Fox | Title unconfirmed (appeared in LinkedIn search for Amber design roles) | Amber Electric | Staff/Principal Product Designer | May be involved in design hiring — verify title before reaching out. https://www.linkedin.com/in/vida-fox/ |
| Dan Adams | Co-founder & Co-CEO | Amber Electric | Staff/Principal Product Designer | Founder-level contact. https://au.linkedin.com/in/dan-adams-615b72a4 |

---

## 📊 Market notes
<!-- Newest first. A dated **YYYY-MM-DD (run N)** line, then one bullet per observation. -->

**2026-09-02 (run 5)**
- Blinq (Melbourne startup, digital business cards) hiring two PD roles concurrently — Staff and Senior.
- Harrison.ai and Sapia.ai have no product design roles open this rotation.
- SafetyCulture's PD role is Sydney-only hybrid — hard filter.
- Jun 2026 ⏳ roles (Canva, UniSuper, Airwallex, Mable, Atlassian) now 11+ weeks old — likely expired; verify and move to outreach targets next run.

**2026-08-31 (run 4)**
- Bupa is actively hiring multiple design roles — Design Lead, Principal Design Researcher, App Designer and Service Designer all surfaced this run.
- Bupa CRM Transformation Design Lead added as ⏳.
- Lorikeet (AI customer support) and New Forests (sustainable forestry asset management) had no product design roles.
- EatClub Senior PD (remote, hospitality tech) scored ~42 — below threshold, discarded.
- All Bupa direct page URLs returned 404 — careers site may be auth-gated or cached differently.
- Jun 2026 ⏳ roles are now 10+ weeks old — treat as likely expired; move to outreach targets next run.

**2026-08-30 (run 3)**
- Dovetail now has an active Senior PD role in Sydney (4 days onsite — hard filter fail; watch for a remote variant).
- Atticus (Melbourne, legal-tech scale-up) added as ⏳.
- Nintex (Melbourne remote, workflow automation + AI) added as ⏳.
- Jun 2026 ⏳ roles (Canva, UniSuper, Airwallex, Mable, Atlassian) are 10+ weeks old — likely expired; check and move to outreach targets next run.

**2026-06-21 (run 2)**
- Disability/aged care healthtech (Mable) added to the radar — domain overlap with Babylon/Unmind makes it worth watching.
- Atlassian Lead PD is live but the careers page isn't fetchable (JS-rendered?) — careers.atlassian.com needs a direct visit.
- Heidi now advertising both Senior PD (🔥) and Head of PD simultaneously — design leadership hire imminent, so the Senior PD's manager should be clear shortly.
- Airwallex advertising 5 concurrent design roles — unusual breadth for one company; the team may be doubling.
- SEEK alerts again ~90% non-product-design noise (textile, graphic, clothing) — dedicated search terms are critical.

**2026-06-17 (run 1)**
- Two calibration-match 🔥 roles live simultaneously (Amber + Cadmus) — unusual and encouraging.
- SEEK alerts dominated by graphic/fashion design noise — dedicated product-design search terms needed.
- Airwallex advertising Staff PD repeatedly (2 Indeed alerts in 2 days) — the role may be hard to fill; possible opportunity.
- AGL Digital PD in SEEK alerts signals the energy sector continuing to invest in digital product, post-Amber trend.

**2026-06-15 (setup)**
- AU AI-health is hiring designers hard — Heidi alone has 10+ open roles including Senior Product Designer and Head of Product Design (Melbourne), signalling a design team being scaled.
- Dovetail (Sydney) currently hiring eng/sales but no design — worth watching.
