...
Work top to bottom. Total time ~45–60 min, most of it the one-time Google OAuth.
...
3. Upload this whole folder's contents (drag-and-drop in the GitHub web UI, or `git push`). Make sure these come across:
   ...
   scripts/  (requirements.txt, get_gmail_token.py, fetch_alerts.py, send_digest.py)
   ...
   .gitignore
   ```
   The `.gitignore` keeps secrets out — never commit `credentials.json` or any token.
...
## Part 3 — Google Cloud OAuth for Gmail (the fiddly one — go slow)

This lets the agent read your `job-scout` label and email the digest to your own Gmail inbox. You do it once.

1. **Create the Gmail label + filter** (in Gmail, not Google Cloud): follow `email-alerts-setup.md` Step 1 — make a label called exactly `job-scout` and a filter routing your job-alert senders into it.
2. **Google Cloud Console** (console.cloud.google.com):
   - Create a new project (top bar → New Project → name it "job-scout").
   - **APIs & Services → Library** → search "Gmail API" → **Enable**.
   - **APIs & Services → OAuth consent screen**:
     - User type: **External** → Create.
     - App name "Job Scout", your email as support + developer contact → Save.
     - **Audience / Test users** → add your own Gmail address as a test user. (Leaving the app in "Testing" is fine — you're the only user.)
   - **APIs & Services → Credentials → Create credentials → OAuth client ID**:
     - Application type: **Desktop app** → name it → Create.
     - **Download JSON**. Save it as `scripts/credentials.json` in your local copy of the repo. **Do not commit it.**
3. **Get your refresh token** (run locally, once):
   ```bash
   cd job-scout-2026
   pip install -r scripts/requirements.txt
   python scripts/get_gmail_token.py
   ```
   A browser opens → sign in with the Gmail account that has the `job-scout` label → approve (you may see an "unverified app" screen — click Advanced → Continue, since it's your own app). The script prints three values:
   ```
   GMAIL_CLIENT_ID=...
   GMAIL_CLIENT_SECRET=...
   GMAIL_REFRESH_TOKEN=...
   ```
   Keep these handy for Part 5. (If it says "no refresh token", revoke at myaccount.google.com/permissions and rerun.)

> Note: the digest is **emailed from this Gmail account to itself**, so it lands in your own inbox (with a notification) — it is never sent to anyone else. You can also read the committed `state/digest-latest.md` in the repo. The OAuth scopes are read-only + send.
...
## Part 5 — Add the four (or six) repo secrets
...
| `GMAIL_CLIENT_ID` | from Part 3 |
| `GMAIL_CLIENT_SECRET` | from Part 3 |
...
## Troubleshooting
- **Run fails at "Fetch emails":** the script is built to never hard-fail on missing secrets — if it does error, check the three `GMAIL_*` secrets and that the `job-scout` label exists.
- **No digest arrived:** the digest emails every *second* run; confirm `state/run-state.json` and that the `gmail.send` scope was granted. Check your inbox (and Spam).
...
- **Token expired:** Google refresh tokens for apps in "Testing" can expire after ~7 days of the consent screen being in Testing mode for some account types. If fetches start failing after a while, publish the consent screen (Production) or rerun `get_gmail_token.py` and update the secret.
